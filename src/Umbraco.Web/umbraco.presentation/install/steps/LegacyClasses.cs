...
namespace umbraco.presentation.install
{
    ...
    public partial class detect : System.Web.UI.UserControl
    {
        ...
        protected bool HasEmbeddedDatabaseFiles
        {
            get
            {
                ...
                if (
                    !File.Exists(Umbraco.Core.IO.IOHelper.MapPath(Path.Combine(Umbraco.Core.IO.IOHelper.ResolveUrl(Umbraco.Core.IO.SystemDirectories.Bin), "System.Data.SqlServerCe.dll"))) ||
                    !File.Exists(Umbraco.Core.IO.IOHelper.MapPath(Path.Combine(Umbraco.Core.IO.IOHelper.ResolveUrl(Umbraco.Core.IO.SystemDirectories.Bin), "SQLCE4Umbraco.dll")))
                    )
                    return false;
                else
                    return true;
    ...
    {
        private string _oldAccessFilePath = Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemDirectories.Data + "/access.xml");
        private string _newAccessFilePath = Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemDirectories.Data + "/access.config");
        ...
        {
            filename = Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemDirectories.Config + "/" + filename);
        ...
        protected void updateChanges_Click(object sender, EventArgs e)
        {
            ...
            if (oldAccessFileExist())
            {
                try
                {
                    File.Move(_oldAccessFilePath, Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemFiles.AccessXml));
        ...
        private XmlNodeList GetExtensions(string filename, string elementName)
        {
            ...
            XmlDocument xsltExt = new XmlDocument();
            xsltExt.Load(Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemDirectories.Config + "/" + filename));
    ...
    public partial class skinning : System.Web.UI.UserControl
    {
        ...
        {
            ph_starterKits.Controls.Add(new UserControl().LoadControl(Umbraco.Core.IO.SystemDirectories.Install + "/steps/Skinning/loadStarterKits.ascx"));
        ...
        {
            steps.Skinning.loadStarterKitDesigns ctrl = (steps.Skinning.loadStarterKitDesigns)new UserControl().LoadControl(Umbraco.Core.IO.SystemDirectories.Install + "/steps/Skinning/loadStarterKitDesigns.ascx");
