			...
			if (Request.QueryString["id"] != null) 
			{
				if (Request.QueryString["id"] != "")  
				{
				    ...
				    try
				    {
                        ...
                        var fileNameThumb = Umbraco.Core.IO.SystemDirectories.Root + fileNameOrg.Replace("." + ext, "_thumb.jpg");
                        image.Controls.Add(new LiteralControl("<a href=\"" + Umbraco.Core.IO.SystemDirectories.Root + fileNameOrg + "\" title=\"Zoom\"><img src=\"" + fileNameThumb + "\" border=\"0\"/></a>"));
				    }
				    catch
				    {
				    }

                    image.Controls.Add(new LiteralControl("<script>\nparent.updateImageSource('" + Umbraco.Core.IO.SystemDirectories.Root + fileName.Replace("'", "\\'") + "','" + altText + "','" + fileWidth.ToString() + "','" + fileHeight.ToString() + "')\n</script>"));
