    ...
    public class library
    {
        ...
        public static XPathNodeIterator AllowedGroups(int documentId, string path)
        {
            ...
            foreach (string role in Access.GetAccessingMembershipRoles(documentId, path))
                xd.DocumentElement.AppendChild(Umbraco.Core.XmlHelper.AddTextNode(xd, "role", role));
            return xd.CreateNavigator().Select(".");
        }
        ...
        public static XPathNodeIterator Split(string StringToSplit, string Separator)
        {
            ...
            {
                XmlNode node = Umbraco.Core.XmlHelper.AddTextNode(xd, "value", id);
        ...
        public static XPathNodeIterator GetPreValues(int DataTypeId)
        {
                ...
                SqlHelper.CreateParameter("@dataTypeId", DataTypeId)))
            {
                while (dr.Read())
                {
                    XmlNode n = Umbraco.Core.XmlHelper.AddTextNode(xd, "preValue", dr.GetString("value"));
                    n.Attributes.Append(Umbraco.Core.XmlHelper.AddAttribute(xd, "id", dr.GetInt("id").ToString()));
        ...
        public static XPathNodeIterator GetDictionaryItems(string Key)
        {
            ...
            try
            {
                ...
                foreach (Dictionary.DictionaryItem item in di.Children)
                {
                    ...
                    {
                        if (languageId != 0)
                            xe = Umbraco.Core.XmlHelper.AddTextNode(xd, "DictionaryItem", item.Value(languageId));
                        else
                            xe = Umbraco.Core.XmlHelper.AddTextNode(xd, "DictionaryItem", item.Value());
                    }
                    catch
                    {
                        xe = Umbraco.Core.XmlHelper.AddTextNode(xd, "DictionaryItem", string.Empty);
                    }
                    xe.Attributes.Append(Umbraco.Core.XmlHelper.AddAttribute(xd, "key", item.key));
            ...
            catch (Exception ee)
            {
                xd.DocumentElement.AppendChild(
                    Umbraco.Core.XmlHelper.AddTextNode(xd, "Error", ee.ToString()));
            }
        ...
        public static XPathNodeIterator GetRelatedNodesAsXml(int NodeId)
        {
            ...
            foreach (Relation r in rels)
            {
                ...
                n.AppendChild(Umbraco.Core.XmlHelper.AddCDataNode(xd, "comment", r.Comment));
                n.Attributes.Append(Umbraco.Core.XmlHelper.AddAttribute(xd, "typeId", r.RelType.Id.ToString()));
                n.Attributes.Append(Umbraco.Core.XmlHelper.AddAttribute(xd, "typeName", r.RelType.Name));
                n.Attributes.Append(Umbraco.Core.XmlHelper.AddAttribute(xd, "createDate", r.CreateDate.ToString()));
                n.Attributes.Append(Umbraco.Core.XmlHelper.AddAttribute(xd, "parentId", r.Parent.Id.ToString()));
                n.Attributes.Append(Umbraco.Core.XmlHelper.AddAttribute(xd, "childId", r.Child.Id.ToString()));
