...
namespace umbraco.cms.businesslogic.skinning
{
    public class Skin
    {
        ...
        {
            string manifest = Path.Combine(Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemDirectories.Masterpages + "/" + alias), "skin.xml");
        ...
        {
            return (System.IO.Directory.GetFiles(Umbraco.Core.IO.IOHelper.MapPath(SkinFolder), "*.master").Count() > 0);
        }

        public bool HasBackupFiles()
        {
            return System.IO.Directory.Exists(Umbraco.Core.IO.IOHelper.MapPath(SkinBackupFolder));
        }
        ...
        {
            DeployTemplateFiles(Umbraco.Core.IO.SystemDirectories.Masterpages);
        }


        public void DeployTemplateFiles(string folder)
        {
            string[] masterFiles = System.IO.Directory.GetFiles(Umbraco.Core.IO.IOHelper.MapPath(SkinFolder), "*.master");

            if (masterFiles.Count() > 0)
            {
                if (!System.IO.Directory.Exists(Umbraco.Core.IO.IOHelper.MapPath(SkinBackupFolder)))
                    System.IO.Directory.CreateDirectory(Umbraco.Core.IO.IOHelper.MapPath(SkinBackupFolder));

                foreach (string master in masterFiles)
                {
                    FileInfo fi = new FileInfo(master);
                    string original = Path.Combine(Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemDirectories.Masterpages), fi.Name);
                    string backup = Path.Combine(Umbraco.Core.IO.IOHelper.MapPath(SkinBackupFolder), fi.Name);
        ...
        public void RollbackTemplateFiles(){
            RollbackTemplateFiles(Umbraco.Core.IO.SystemDirectories.Masterpages);
        }
        ...
        public void RollbackTemplateFiles(string folder)
        {
            ...
            if (masterFiles.Count() > 0)
            {
                ...
                {
                    FileInfo fi = new FileInfo(master);
                    string inUse = Path.Combine(Umbraco.Core.IO.IOHelper.MapPath(Umbraco.Core.IO.SystemDirectories.Masterpages), fi.Name);
            ...
            string[] backedUpmasterFiles = System.IO.Directory.GetFiles(Umbraco.Core.IO.IOHelper.MapPath(SkinBackupFolder), "*.master");
        ...
        public string SkinFolder
        {
            get
            {
                return Umbraco.Core.IO.SystemDirectories.Masterpages + "/" + Alias;
            }
        }
